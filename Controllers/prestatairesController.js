...
export const createPrestataire = (req, res) => {
  ...
  const columns = PrestatairesModel.columns.filter(
    (c) => c !== "ID_Prestataires"
  );
  const values = columns.map(
    (col) => data[col] ?? PrestatairesModel.defaults[col] ?? null
  );

  const placeholders = "(" + columns.map(() => "?").join(",") + ")";
  const sql = `INSERT INTO ${PrestatairesModel.table} (${columns.join(
    ","
  )}) VALUES ${placeholders}`;
  ...
  db.query(sql, values, (err, result) => {
    ...
    return res
      .status(201)
      .json({ message: "Prestataire ajouté avec succès", id: result.insertId });
  });
};
...
export const getUrssafParams = (req, res) => {
  ...
  db.query(sql, (err, results) => {
    if (err) {
      console.error(
        "Erreur lors de la récupération des paramètres URSSAF :",
        err
      );
      return res
        .status(500)
